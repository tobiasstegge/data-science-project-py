...
def naive_bayes(train_other, test_other, train_target, test_target):
    ...
    for model in models:
        ...
        models[model].fit(train_other, train_target)
        prediction_target = models[model].predict(test_other)
        yvalues.append(accuracy_score(test_target, prediction_target))
...
def knn(train_other, train_target, test_other, test_target, t):
    ...
    acc_score = accuracy_score(test_target, prediction_test)
    f1 = f1_score(test_target, prediction_test, pos_label='Danger', average="micro")
    print(f'acc {round(acc_score, 10)}')
...
def mlp(train_other, train_target, test_other, test_target, train):
    ...
    lr_type = ['constant']
    max_iter = [85, 90, 95, 100, 105, 110, 115, 120]
    learning_rate = [.1, .5, .9]
    ...
    plot_overfitting_study(max_iter, y_trn_values, y_tst_values, name=f'NN_lr_type={lr_type}_lr={lr}',
                           xlabel='nr episodes', ylabel=str(eval_metric), filename=file_tag)